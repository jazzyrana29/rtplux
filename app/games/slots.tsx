...
const SlotsGameContent: React.FC = () => {
  const phaserRef = useRef<Phaser.Game | null>(null);
  const containerId = 'slots-phaser-container';
    ...
    try {
      ...
      if (isFocused && iframeLoaded && iframeRef.current) {
        ...
        try {
          ...
          trackGameError(phaserError as Error, 'slots', {
  ...
  const handleIFrameLoad = () => {
    try {
      ...
      trackUserAction('iframe_loaded', { game: 'slots' });
    } catch (error) {
    ...
    >
                ...
                >
                  ...
                  >
                    ...
                    <AnimatedText className="text-yellow-400 text-xl font-bold mb-2">
                      {t(SLOTS_CONSTANTS.LOADING_TABLE)}
                    </AnimatedText>
                    <motion.div
          ...
          <>
            ...
            >
              <AnimatedText className="text-2xl font-bold text-casino-gold mb-4 text-center">
                {t(SLOTS_CONSTANTS.HOW_TO_PLAY_TITLE)}
              </AnimatedText>
              ...
              >
                <div>
                  <AnimatedText className="font-bold text-yellow-400 mb-2">
                    {t(SLOTS_CONSTANTS.BASIC_CONTROLS)}
                  </AnimatedText>
                  <AnimatedText className="text-sm">
                    {t(SLOTS_CONSTANTS.BASIC_CONTROLS_TEXT)}
                  </AnimatedText>
                </div>

                <div>
                  <AnimatedText className="font-bold text-yellow-400 mb-2">
                    {t(SLOTS_CONSTANTS.BETTING_OPTIONS)}
                  </AnimatedText>
                  <AnimatedText className="text-sm">
                    {t(SLOTS_CONSTANTS.BETTING_OPTIONS_TEXT)}
                  </AnimatedText>
                </div>
              </motion.div>
      ...
      >
        <AnimatedText className="text-gray-300 text-sm">
          {t(SLOTS_CONSTANTS.RESPONSIVE_TABLE)}
        </AnimatedText>
        <motion.div
          animate={{
            opacity: [0.5, 1, 0.5],
        ...
        >
          <AnimatedText className="text-yellow-400 text-xs mt-1">
            {t(SLOTS_CONSTANTS.RESIZE_WINDOW)}
          </AnimatedText>
        </motion.div>
      </motion.div>
    </AnimatedView>
...
export default SlotsGameContent;
